...
# Run command ./aws_job_run_cookiemonstr_model_readfilters.sh
...
aws s3 cp ../pipeline/run_cookiemonstr_model_readfilters.sh s3://ssc-gangstr-denovos/scripts/run_cookiemonstr_model_readfilters.sh || echo "run_cookiemonstr_model_readfilters.sh upload to s3 failed"

while read -r FAMID CRAMSLIST; do
     # RUN=false
     # ### Check .denovos.all_mutations.tab files exsits - if exist, then do not re-run CookieMonSTR
     # FILE=denovos_GW_priors_Jan20/${PHASE}/${FAMID}_allchrs.denovos.filtered_mutations.tab
     ...
     # if (${RUN}); then
       ...
       aws batch submit-job \
           ...
           --container-overrides 'command=["run_cookiemonstr_model_readfilters.sh",'"${FAMID}"', '"${SSC_PARAMS}"'],environment=[{name="BATCH_FILE_TYPE",value="script"},{name="BATCH_FILE_S3_URL",value="s3://ssc-gangstr-denovos/scripts/run_cookiemonstr_model_readfilters.sh"}]'
     # fi
done <${SSC_FILE}
